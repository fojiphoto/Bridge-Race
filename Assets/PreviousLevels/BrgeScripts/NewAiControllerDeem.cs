...
public class NewAiControllerDeem : MonoBehaviour
{
    ...
    void Update()
    {
        if (move)
        {
            ...
            if (movetoplace)
            {
                ...
                {
                   // nthstep.transform.root.GetComponent<Enemyr_Bridge>().thisBridggingRacechar(gameObject);
                    nthstep.transform.root.GetComponent<NewEnemyBridge>().thisBridggingRacechar(gameObject);
                    movetoplace = false;
                    stage++;
                    TakeBrgR_thismany();
                    addBrgR_stairscases();
        ...
        if (collision.gameObject.CompareTag("Bot") || collision.gameObject.CompareTag("Player"))
        {
            if (!movetoplace)
            {
                if (Bag.transform.childCount > 0)
                {
                    ...
                    {
                        //if (collision.gameObject.transform.GetComponent<BridgeControllerr_Bridge>().Bag.transform.childCount > Bag.transform.childCount)
                        //{
                        //    StartCoroutine(BrgR_stand());
                        //}
                        if (collision.gameObject.transform.GetComponent<NewBridgePlayerController>().Bag.transform.childCount > Bag.transform.childCount)
                        {
                            StartCoroutine(BrgR_stand());
                        }
                    }
    ...
    public void addBrgR_stairscases()
    {
        if (stage <= 2)
        {
            ...
            //mytargetparents[stage].GetComponent<ColorPlacer_Bridge>().assigncolor(gameObject, "bot");
            mytargetparents[stage].GetComponent<NewColorPlacerDeem>().assigncolor(gameObject, "bot");
